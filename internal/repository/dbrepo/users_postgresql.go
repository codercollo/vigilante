...
func (m *postgresDBRepo) Authenicate(email, testPassword string) (int, string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	query := `
	   select id, password, user_active from users where email = $1 and delete_at is null
	`
...
func (m *postgresDBRepo) InsertRememberMeToken(id int, token string) error {
	...
	stmt := `insert into remember tokens (user_id, remember_token) values($1, $2)`
...
func (m *postgresDBRepo) DeleteToken(token string) error {
	...
	stmt := `
	delete from remember_tokens where remember_token = $1`
...
func (m *postgresDBRepo) CheckForToken(id int, token string) bool {
	...
	stmt := `SELECT id FROM remember_tokens where user_id = $1 and remember=token = $2`
...
func (m *postgresDBRepo) DeleteUser(id int) error {
	...
	stmt := `
	 update users set deleted_at = $1, user_active = 0 where id = $2
	`
